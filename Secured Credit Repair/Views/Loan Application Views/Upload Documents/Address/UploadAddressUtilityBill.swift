...
struct UploadAddressUtilityBill: View {
    ...
    var body: some View {
        VStack {
            ...
            Button(action:{}) {
                NavigationLink(destination: LoanUploadAddressDocuments(loanApplicationService: loanApplicationService), isActive: $backToDocumentsUploadView)
                {
                ZStack {
    ...
    private var isContinueButtonActive: Binding<Bool> { Binding (
        get: { loanApplicationService.loanApplicationForm.Documents.UtilityBill.count >= 1 },
        set: { if !$0 { backToDocumentsUploadView = true; loanApplicationService.UtilityBillStepCompletedIcon = "checkmark" } }
        )
    }
